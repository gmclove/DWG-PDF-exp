...
def main():
    ...
    project_name = input(f"Enter Project Name [{default_project_name}]: ").strip() or default_project_name

    recursive_input = input("Include subfolders when searching for DWG files? (Y/N): ").strip().lower()
    recursive = recursive_input.startswith('y')

    print("Configure Title Block Settings")
    ...
    print(f"Scanning for DWGs in: {input_dir}")
    ...
    print(f"Copying DWGs to: {dwg_out_dir}")
    ...
    try:
        with AutoCADPdfConverter(pdf_pc3_name="DWG To PDF.pc3", visible=False, tb_config=TB_CONFIG) as converter:
            print("Generating Individual PDFs and building Drawing List...")
            total = len(copied)
        ...
        print(f"ERROR: AutoCAD conversion failed: {e}")
        if drawing_rows:
            write_drawing_list_csv(drawing_rows, drawing_list_csv)
        sys.exit(2)

    if all_pdfs:
        print(f"Merging {len(all_pdfs)} PDFs into one combined file...")
        ...
        print("No PDFs generated to merge.")
